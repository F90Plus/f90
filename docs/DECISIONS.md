...
### D-059 — Phase 2 CANONICAL CLOSEOUT: docs reconciled to the live state; product + Phase-3 readiness captured ✅ (2026-06-06)
**Context:** the founder independently verified the live system — `settle_fixture(text)` present in prod, migration 0005 applied, **settlement path installed, loop live** — and asked for a final end-to-end reconciliation + clean handoff. **All prior operator-runbook items are CLOSED.**
**Canonical status (authoritative as of 2026-06-06):**
- **Phase 2 = COMPLETE · DEPLOYED · VERIFIED.**
- **Prediction loop = LIVE + VERIFIED** (72 fixtures + real predictions in prod; `make_prediction` server-authoritative).
- **Settlement = INSTALLED + VERIFIED** (`settle_fixture` present; recurring op = `POST /api/admin/settle` per matchday from the opener **11 Jun**).
**Actions taken (docs + memory only — no code change):**
- **Reconciled all current-state docs to reality**, removing stale "loop not live / operator runbook pending" wording (historical ADR entries left intact — superseded by D-058): `PHASE_2_HANDOFF.md` top banner → "LIVE + VERIFIED", runbook header → "EXECUTED" (kept as the recurring settle reference), stale caveats fixed (D-035 done · ESLint fixed); `PROJECT_STATE.md` body Phase-2 banner + resume entry → loop live; `ROADMAP.md` Phase-2 heading + loop line → "COMPLETE · DEPLOYED · LOOP LIVE + VERIFIED".
- **Product observation logged** — new [PRODUCT_OBSERVATIONS.md](PRODUCT_OBSERVATIONS.md) **OBS-001**: the prediction-card UX does not make the core action dominant/scannable enough (less so than Polymarket / sportsbook cards). **The primary product concern is now participation-UX, not infrastructure.** Documented as a future design initiative — **not implemented**; hard constraint = clarity only, **D-037 binding** (no odds/wager framing).
- **Phase-3 readiness captured** — new [PHASE_3_READINESS.md](PHASE_3_READINESS.md): foundation ready; the only hard gate is the **free-staking brand ratification (founder)**. Assessment: free/variable staking risks **sportsbook drift** ("choose how much to risk" vs D-037 + the free/no-betting invariant) and the skin-in-the-game value is already met by D-054 → **recommend NOT adopting it as a risk gesture**; if ever pursued, reframe inside the no-betting language with explicit founder ratification. Phase 3 can be planned/built free-to-play without resolving it.
**Repo state:** `main` == `origin/main`; only `main`; tree clean.
**Remaining (non-blocking):** (1) **Resend SMTP** for magic-link at scale = recommendation (Google OAuth works); (2) recurring `settle` per matchday = operational, not a blocker. **No hard blockers remain for Phase 2; Phase 3 awaits the founder's free-staking decision.**
