...
function GoogleSubmit() {
  ...
  return (
    ...
    >
      <GoogleIcon />
      {pending ? t('google.connecting') : t('google.label')}
    </button>
  );
}

/** Google OAuth trigger. `action` is the bound signInWithGoogle (redirects). */
export function GoogleButton({ action }: { action: GoogleAction }) {
  return (
    <form action={action}>
      <GoogleSubmit />
    </form>
